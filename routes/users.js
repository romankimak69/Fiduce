...
router.post('/register',rateLimiterMiddleware,ensureNotAuthenticated, async (req, res) => {
  const { name } = req.body;
...
router.patch('/password',rateLimiterMiddleware,ensureNotAuthenticated, resToJson(async (req, res) => {
  const { username } = req.body;
